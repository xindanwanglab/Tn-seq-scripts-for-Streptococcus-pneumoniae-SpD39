...
for line in open(sys.argv[2]):
    ...
    if strand == '+':
        site = int(split[3]) + 16  #if on + strand, take column 3 position and add 1bp)
        if site in chrITA: chrITA[site][0] += 1  #if read has been found before, tally 1 more in F reads
    if strand == '-':
        site = int(split[3]) + 0 #if on - strand, add col 3 position to length of read in position 4 and add 1
